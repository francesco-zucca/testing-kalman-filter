...
Motivation: breaking the assumption of normally distributed observable data, assigning them 2-3 different distribution functions:
...
see what happens as we increase n from 10-100-1000-10000, etc

## 2.1 Gaussian Distribution

```{r}
mean_W875 <-   mean(df_clean$W875RX1)
mean_indp <-   mean(df_clean$INDPRO)
mean_cmrm <-   mean(df_clean$CMRMTSPL)
mean_payems <- mean(df_clean$PAYEMS)

sd_W875 <-   sd(df_clean$W875RX1)
sd_indp <-   sd(df_clean$INDPRO)
sd_cmrm <-   sd(df_clean$CMRMTSPL)
sd_payems <- sd(df_clean$PAYEMS)
...
f_true <- numeric(TT)
for (t in 3:TT)
  f_true[t] <- 0.6*f_true[t-1] + 0.2*f_true[t-2] + rnorm(1)
...
# run kalman
final_sim <- kalman(opt_res$par, Y_normal, n = 4, TT = nrow(Y_normal), varQ)
f_hat <- final_sim$filter_mat[, 1]

# check recovery
cat("Factor recovery correlation:", cor(f_hat, f_true), "\n")
```
...
n = 800
# standardized so variance = 1 
rt(n, df = 5) / sqrt(5 / (5 - 2))
```


## 2.3 Skewed-Elliptical

```{r skew-t}
library(sn)
rst(n, xi=0, omega=1,  # alpha controls skew (positive <==> skews right)
    alpha=5, nu=5) 

?rt
?replicate
?sapply
vignette("sn")  # for the skew-t package
```

1. student-t (realistic given the finite-sample nature of data)
2. skewed-elliptical
3. skewed-symmetric


show that results are unbiased (should be roughly the same across all distributions) but NOT efficient (show variance-covariance matrix, explain why)
