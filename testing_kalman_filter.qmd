...
Despite the violation of the Gaussianity assumption, as long as the error presents finite variance, the Kalman filter remains the Best Linear Unbiased Estimator (BLUE). Hence, while it loses efficiency, it still extracts an unbiased estimate of the latent factor. Chapter 2 will be dedicated to an empirical demonstration of the robustness of the Kalman filter, by simulating an the economic model proposed by S&W under increasingly more elliptical distributions.
