...
Noticeably, to capture the cyclic pattern of business cycles, both the factor and the error follow an $AR(2)$ process. Visually, the model can be represented by the graphical model represented in @fig-SW.
...
## 1.1. Latent factor estimation {#sec-kalman-filter}
...
```{r save fit1, eval=TRUE, echo=FALSE}
# save plot image
...
The dynamic factor model proposed by Stock and Watson explicitly assumes that all structural shocks are purely Gaussian, which grants it the property of being the minimum mean square error (MMSE) estimator of the latent factor, as proven by @hamilton1994 [Section 13.2].[^1] However, economic cycles frequently feature sudden and massive shocks (e.g. the COVID-19 crisis), hence macroeconomic indicators are rarely normally distributed. Instead, they tend to exhibit heavy-tailed behavior. As illustrated in the Quantile-Quantile (Q-Q) plot below, the distribution of our indicators indeed deviates from normality, confirming the presence of fat tails.

[^1]: This follows from the fact that the model is linear and that shocks $w_t$ and $\epsilon_t$ are assumed to be Gaussian and independent; the conditional expectation $\mathbb{E}[h_t | y_{1:t}]$ minimizes mean squared error among all measurable functions of $y_{1:t}$. See @hamilton1994 \[Section 4.1\] for a standard proof.
...
## 2.1 Gaussian Distribution {#sec-gaussian-dist}
...
For each sample size, we generate a true latent factor following an AR(2) process with coefficient values of $\hat{\phi}_1 = s$ `r fit1$phi$est[1]` and $\hat{\phi}_2 = s$ `r fit1$phi$est[2]`, estimated from the S&W model in @sec-kalman-filter, with Gaussian shocks by assumption. We then construct the observable data by multiplying the true factor by the estimated factor loadings from @sec-kalman-filter and adding normally distributed idiosyncratic errors, replicating the structure of the S&W measurement equation.
...
```{r save gaussian, echo=FALSE, eval=TRUE}
# save sim results
save(results1, file = "results/gaussian_results.RData")
```

```{r gaussian plotting data, echo=TRUE}
# load in data
...
```{r compute convergence value, echo=FALSE, eval=TRUE}
...
```{r save gaussian plot, eval=TRUE, echo=FALSE}
# save plot image
...
## 2.2 Student-t Distribution {#sec-student-t-dist}
...
```{r student_t test run, eval=TRUE}
# one iteration
TT <- seq(10, 10000, by = 50)
results2 <- data.frame(
  sample_size = TT,
...
```{r save student t, echo=FALSE, eval=TRUE}
# save sim results
...
```{r save student-t plot, eval=TRUE, echo=FALSE}
# save plot image
ggsave(filename = "images/student_t_plot.png",
       width=8, height=5, dpi=300)
```

As illustrated in @fig-student-t, the Kalman filter remains consistent under student-t shocks, with factor recovery converging to approximately `r asymptotic_cor` as $T \rightarrow \infty$, in line with the Gaussian baseline from @sec-gaussian-dist.
...
## 2.3 Skew-t Distribution {#sec-skew-t-dist}
...
`factor_shocks <- rst(tt, xi=0, omega=1, alpha=5, nu=5)` `factor_shocks <- factor_shocks - mean(factor_shocks)`
...
```{r save skew_t, echo=FALSE, eval=TRUE}
# save sim results
...
```{r save skew-t plot, eval=TRUE, echo=FALSE}
# save plot object
...
```{r skew-t increasing alphas sim, eval=TRUE}
# one iteration
...
```{r save skew t alphas, echo=FALSE, eval=TRUE}
# save sim results
...
```{r save variance table, echo=FALSE, eval=TRUE}
# save table
...
## 3.1. Using Parallel Analysis to Determine the Number of Factors {#sec-one-factor-specification}
...
```{r save parallel1 plot, eval=TRUE, echo=FALSE}
# save plot object
...
```{r save parallel2 plot, eval=TRUE, echo=FALSE}
# save plot object
...
```{r save fit2 plot, eval=TRUE, echo=FALSE}
# save plot object
...
#| fig-cap: "Maximum likelihood estimates of the misspecified augmented model."
...
```{r save fit3 plot, eval=TRUE, echo=FALSE}
# save plot object
...
To achieve identification, we impose a lower-triangular restriction on the loading matrix $\Lambda$. This is because a factor model $y = \Lambda f + e$ is invariant to orthogonal rotations $Q$, as:
