...
n_lam <- r*n - r*(r - 1)/2 # Number of loadings
...
kalman <- function(th, y, n, r, TT, n_lam, h_dim, varQ = 1) {
...
# Pulling starting values
th0 <- make_startval(y, n, r, n_lam)

# Function that maximizes the likelihood
opt <- optim(
  par    = th0,
  fn     = function(th) kalman(th, y, n, r, TT, n_lam, h_dim, varQ = 1)$neg_loglik,
  method = "BFGS",
...
H <- optimHess(
  par = th_hat,
  fn  = function(th) kalman(th, y, n, r, TT, n_lam, h_dim)$neg_loglik
)
...
lambda_hat <- th_hat[idx_lam]; lambda_se <- se_hat[idx_lam]
phi_hat    <- th_hat[idx_phi]; phi_se    <- se_hat[idx_phi]
psi_hat    <- th_hat[idx_psi]; psi_se    <- se_hat[idx_psi]
sigma_hat  <- th_hat[idx_sig]; sigma_se  <- se_hat[idx_sig]

# Rerunning Kalman filter with optimal parameters to extract the factors
kf_hat     <- kalman(th_hat, y, n, r, TT, n_lam, h_dim)
