...
The results of parallel analysis on the expanded dataset are reported in @fig-parallel2. Notably, both the first and the second observed eigenvalues exceed the AR(2) noise benchmark. The first eigenvalue is approximately $3.158$ (versus a benchmark of $1.197$), and the second is around $2.606$ (against $1.112$). This analysis suggests that the four indicators we have added to the dataset share a common latent factor that reflects price pressures in the economy, which is distinct from the real business cycle factor that drives the comovements in the original four indicators.
...
As shown in Figure X, the single factor estimated on the augmented eight-indicator dataset continues to track real economic activity well. Its fit to actual GDP deteriorates to an $R^2$ of $0.711$ and an MSE of $0.336$, worse than the baseline model's $R^2$ of $0.763$ and MSE of $0.276$. Moreover, despite including *PCEPI* in the indicator set, the estimated factor fails to capture inflation, yielding an $R^2$ of just $0.011$ against monthly *PCEPI* growth.
...
tibble(
  ...
  Group     = c(rep("Real", 3), rep("Price", 3)),
  Lambda    = lambda_hat
)
```
